...
def _blog_card(item) -> rx.Component:
    return rx.el.div(
        rx.cond(
            item.cover_url,
            rx.el.img(src=item.cover_url, alt=item.title, class_name="ag-blog-card-img"),
            rx.el.div(
                rx.el.i(class_name="bi bi-journal-text", style={"font-size": "3rem", "color": "var(--ag-gray)"}),
                style={"height": "180px", "background": "var(--ag-bg)", "display": "flex",
                       "align-items": "center", "justify-content": "center"},
            ),
        ),
        rx.el.div(
            rx.el.div("Блог", class_name="ag-blog-cat"),
            rx.el.h5(
                rx.el.a(item.title, href="/blog/" + item.slug,
            ...
            rx.el.div(
                rx.el.span(rx.el.i(class_name="bi bi-person me-1"), item.author,
            ...
            rx.el.a("Читать далее ", href="/blog/" + item.slug, class_name="ag-read-more"),
...
def _sidebar() -> rx.Component:
    return rx.el.aside(
        ...
        rx.el.div(
            rx.el.div("Свежие статьи", class_name="ag-sidebar-widget-title"),
            rx.foreach(
        ...
        rx.el.ul(
            rx.el.li(
                rx.el.button(rx.el.i(class_name="bi bi-chevron-left"),
                             on_click=ContentState.set_page_blog(ContentState.page - 1), class_name="page-link"),
...
def blog_page() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.div(
            rx.el.div(
                rx.el.span(rx.el.a("Главная", href="/"), " / Блог", class_name="ag-breadcrumb"),
                class_name="mb-2",
            ),
            rx.el.h1("Блог Auto Hub"),
        ...
        rx.el.section(
                    ...
                    rx.el.div(
                        rx.cond(
                            ...
                            rx.el.div(
                                rx.foreach(
                                    ContentState.articles,
                                    lambda item: rx.el.div(_blog_card(item), class_name="col-md-4"),
                                ),
                                class_name="row g-4",
                            ),
                        ),
            ...
            on_mount=ContentState.load_blog,
        ),
        _footer(),
    )
